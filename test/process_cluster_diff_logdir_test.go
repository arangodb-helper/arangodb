...
func TestProcessClusterDifferentLogDirNoLog2File(t *testing.T) {
	...
	masterLogDir := filepath.Join(logDir, "master")
	master := Spawn(t, "${STARTER} --log.file=false --log.dir="+masterLogDir+" "+createEnvironmentStarterOptions())
	defer master.Close()

	slave1LogDir := filepath.Join(logDir, "slave1")
	dataDirSlave1 := SetUniqueDataDir(t)
	defer os.RemoveAll(dataDirSlave1)
	slave1 := Spawn(t, "${STARTER} --log.file=false --starter.join 127.0.0.1 --log.dir="+slave1LogDir+" "+createEnvironmentStarterOptions())
	defer slave1.Close()

	slave2LogDir := filepath.Join(logDir, "slave2")
	dataDirSlave2 := SetUniqueDataDir(t)
	defer os.RemoveAll(dataDirSlave2)
	slave2 := Spawn(t, "${STARTER} --log.file=false --starter.join 127.0.0.1 --log.dir="+slave2LogDir+" "+createEnvironmentStarterOptions())
	defer slave2.Close()
