...
func newTestSingleService(t *testing.T, version string) *Service {
	...
	s := &Service{
		...
		databaseFeatures: NewDatabaseFeatures(driver.Version(version), false),
	}
	s.runtimeClusterManager.myPeers = ClusterConfig{
		AllPeers: []Peer{
			newPeer(id, "127.0.0.1", 8528, 0, dataDir, preparePeerServers(ServiceModeSingle, BootstrapConfig{}, Config{}, nil), false),
		},
	}
	return s
}
