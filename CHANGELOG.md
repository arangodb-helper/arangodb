...
- CircleCI / Makefile: when **`update_starter_latest_and_rolling_tags=false`** (local: **`STARTER_TAGS_SKIP_LATEST=1`**), Docker push skips **`:latest`** only; **`:<VERSION>`**, **`:x.y`**, and **`:x`** are still published (previously only **`:<VERSION>`** was pushed).
